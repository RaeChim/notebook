...
&emsp;&emsp;对于一个向量$$ \mathbf{a} \in \mathbf{R}^n ,\ \mathbf{a} \neq 0 $$和一个标量$$ \beta \in \mathbf{R} $$，定义$$ \color{green}{ H = \{ \mathbf{x} \in \mathbf{R}^n | \mathbf{a}^T \mathbf{x} \} } $$为<font color="#3FBF3F">hyperplane（超平面）</font>。

<center>
    <figure>
        <img src="../../notebook\part5\images\2-1.JPG" width=450px>
    </figure>
</center>
...
**Property 3**：标准的LP的可行域$$ P = \{ \mathbf{x} \in \mathbf{R}^n | \mathbf{Ax=b} ,\ \mathbf{x} \ge 0 \} $$为多边集合。

**Property 4**：如果$$ P \neq \emptyset $$且$$ \exists \beta \in \mathbf{R} $$，这样$$ P \subset H_L := \{ \mathbf{x} \in \mathbf{R}^n | -\mathbf{c}^T \mathbf{x} \le \beta $$那么$$ \min_{\mathbf{x} \in P} \mathbf{c}^T \mathbf{x} \ge \beta $$。  
&emsp;&emsp;进一步地，如果$$ \mathbf{x}^* \in P \cap H $$，那么$$ \mathbf{x}^* \in P^* $$。
