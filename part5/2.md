...
&emsp;&emsp;Case 2: $$P$$没有一个extremal direction $$\mathbf{d}$$使得$$\mathbf{c}^T \mathbf{d} \lt 0$$，那么$$\forall \mathbf{x} \in P $$，要么$$\mathbf{x} = \sum_{i \in I}\lambda_i v^i \text{ with } \sum_{i \in I}\lambda_i = 1,\ \lambda_i \ge 0 $$，要么$$\mathbf{x} = \sum_{i \in I}\lambda_i v^i + \overline{\mathbf{d}} \text{ with } \mathbf{c}^T \mathbf{x} \overline{\mathbf{d}} \ge 0 $$。这两种情况下，
