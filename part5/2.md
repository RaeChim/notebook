...
&emsp;&emsp;对于一个向量$$ \mathbf{a} \in \mathbf{R}^n ,\ \mathbf{a} \neq 0 $$和一个标量$$ \beta \in \mathbf{R} $$，定义$$ \color{green}{ H = \{ \mathbf{x} \in \mathbf{R}^n \lvert \mathbf{a}^T \mathbf{x} = \beta \} } $$为<font color="#28a745">hyperplane（超平面）</font>。
...
&emsp;&emsp;如果$$ \sum_{i=1}^P \lambda_i = 1 $$，那么$$ \mathbf{x} $$为$$ \{ \mathbf{x}^1, \mathbf{x}^2, \ldots, \mathbf{x}^P \} $$的一个<font color="#00B050">affine combination</font>（仿射组合）。

&emsp;&emsp;如果$$ \lambda_i \ge 0 $$，那么$$ \mathbf{x} $$为$$ \{ \mathbf{x}^1, \mathbf{x}^2, \ldots, \mathbf{x}^P \} $$的一个<font color="#00B050">conic combination</font>（圆锥组合）。

&emsp;&emsp;如果$$ \sum_{i=1}^P \lambda_i = 1 ,\ \lambda_i \ge 0 $$，那么$$ \mathbf{x} $$为$$ \{ \mathbf{x}^1, \mathbf{x}^2, \ldots, \mathbf{x}^P \} $$的一个<font color="#00B050">convex combination</font>（凸组合）。

#### Definition: affine set, convex set and cone

令$$ S $$为$$ \mathbf{R}^n $$的一个子集

&emsp;&emsp;如果$$ S $$中任意两点的仿射组合落在$$ S $$内，那么$$ S $$就是一个<font color="#00B050">affine set</font>。

&emsp;&emsp;如果$$ S $$中任意两点的凸组合落在$$ S $$内，那么$$ S $$就是一个<font color="#00B050">convex set</font>。

&emsp;&emsp;如果对于所有的$$ \mathbf{x} \in S $$且$$ \lambda \ge 0 $$都有$$ \lambda \mathbf{x} \in S $$，那么$$ S $$就是一个<font color="#00B050">cone</font>（锥）。
