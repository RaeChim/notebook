...
&emsp;&emsp;$$\mathbf{M}^{-1}$$（或$$\mathbf{M}$$）是LP的<font color="#28a745">fundamental matrix</font>，$$\mathbf{M}^{-1} = \begin{bmatrix}\mathbf{B}^{-1} & -\mathbf{B}^{-1}\mathbf{N} \\ 0 & \mathbf{I} \end{bmatrix}$$。因此，当$$\mathbf{B}^{-1}$$已知时就可以得到$$\mathbf{M}^{-1}$$。
