...
<b><font color="#00B050">体渲染</font></b>：视线上$$\mathbf{r}$$所有的点投射到图像上形成像素颜色$$C$$的过程：
...
* 用途：记录辐射场信息
* 输入：坐标位置$$\bm{x}$$的位置编码以及视角向量$$\bm{d}$$的位置编码
* 输出：在坐标位置$$\bm{x}$$的体密度$$\sigma$$以及视线$$\bm{d}$$方向观察时$$\bm{x}$$处的的RGB值$$C$$
...
$$\gamma(\cdot)$$独立应用于待编码向量的各个维度，对于位置向量$$\bm{x}$$编码时$$L$$取10，对于视角向量$$\bm{d}$$编码时$$L$$取4。
经过位置编码，$$\bm{x}$$的编码$$\gamma(\bm{x})$$维度为60，$$\bm{d}$$的编码$$\gamma(\bm{d})$$维度为24。
