...
&emsp;&emsp; 通过引入拉格朗日乘数向量$$ \mathbf{\lambda}(t) $$（也称协状态向量）和利用式(2.7.6)，可以得到性能指标在最优条件下为
...
$$\begin{aligned}
 ...
 = \int_{t_0}^{t_f}
      \big \{
          ...
          + \left( \frac{\partial S}{\partial \mathbf{x}} \right)'_* 
            \mathbf{\dot{x}}^*(t)
...
$$\begin{aligned}
    \mathcal{L}
= & \mathcal{L} 
		(\mathbf{x}^*(t),\mathbf{\dot{x}}^*(t),\mathbf{u}^*(t),\mathbf{\lambda}(t),t) \\
= & V(\mathbf{x}^*(t),\mathbf{u}^*(t),t) 
  + \left( \frac{\partial S}{\partial \mathbf{x}} \right)'_*
	\mathbf{\dot{x}}^*(t)
  + \frac{\partial S}{\partial t} 
